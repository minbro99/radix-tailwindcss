...
const PostForm: React.FC<PostFormProps> = ({ postToEdit, onSave }) => {
  ...
  return (
    <form onSubmit={handleSubmit}>
      <h2 className='text-white'>{postToEdit ? 'Edit Post' : 'New Post'}</h2>
      <div className='bg-slate-500 text-white p-4 rounded'>
        <label className=' block mb-2'>Title:</label> {/* 텍스트 색상 흰색으로 변경 */}
        <input
          value={title}
      ...
      <div className='bg-black text-white p-4 rounded mt-4'>
        <label className='text-white block mb-2'>Content:</label> {/* 텍스트 색상 흰색으로 변경 */}
        <textarea
          value={content}
      ...
      <div className='bg-black text-white p-4 rounded mt-4'>
        <label className='text-white block mb-2'>Author:</label> {/* 텍스트 색상 흰색으로 변경 */}
        <input
          value={author}
